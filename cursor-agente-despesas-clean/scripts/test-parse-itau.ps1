...
$OUTPUT_JSON = if ($env:OUTPUT_JSON) { $env:OUTPUT_JSON } else { "parse_output.json" }
...
try {
    ...
    try {
        ...
        $byteArrayContent = [System.Net.Http.ByteArrayContent]::new([byte[]]$fileBytes)
    ...
    try {
        ...
        if ($jsonOutput) {
            ...
            # Também escrever no pipeline (stdout) para suportar redirecionamento
            Write-Output $jsonOutput
            
            # Salvar JSON em arquivo (sem BOM para compatibilidade)
            try {
                $outputPath = if ([System.IO.Path]::IsPathRooted($OUTPUT_JSON)) {
                    $OUTPUT_JSON
                } else {
                    Join-Path (Get-Location) $OUTPUT_JSON
                }
                ...
                Write-Host "Aviso: Nao foi possivel salvar JSON em arquivo: $($_.Exception.Message)" -ForegroundColor Yellow
            }
        } else {
        ...
        if ($response) {
            $altOutput = $response | Out-String
            Write-Host $altOutput
            Write-Output $altOutput
        }
    }
    Write-Host ""
    ...
    if ($response.stats) {
        ...
        if ($response.stats.PSObject.Properties.Name -contains "sum_saida") {
            Write-Host "     Sum saida: $($response.stats.sum_saida)" -ForegroundColor Gray
        }
        if ($response.stats.PSObject.Properties.Name -contains "sum_entrada") {
            Write-Host "     Sum entrada: $($response.stats.sum_entrada)" -ForegroundColor Gray
        }
