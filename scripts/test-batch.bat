@echo off
REM Script para testar classificação de lote de transações (Windows)
REM Executa POST /v1/classify com app/samples/tx_batch.json

echo 🎯 Testando classificação de lote de transações...

REM Verificar se o arquivo de exemplo existe
if not exist "app\samples\tx_batch.json" (
    echo ❌ Arquivo app\samples\tx_batch.json não encontrado!
    exit /b 1
)
...
echo 📄 Arquivo: app\samples\tx_batch.json
echo.

curl -s -X POST "http://localhost:8080/v1/classify" ^
  -H "Content-Type: application/json" ^
  -d @app\samples\tx_batch.json
